...
class Trainer(nn.Module):
    ...
    def log_loss(self, logger, n_iter):
        logger.log({"loss/total": self.loss_gen.item() + self.loss_dis_gp.item(), "n_iter": n_iter + 1})
        logger.log({"loss/recon": self.loss_recon.item(), "n_iter": n_iter + 1})
        logger.log({"loss/class": self.loss_class.item(), "n_iter": n_iter + 1})
        logger.log({"loss/adv": self.loss_adver.item(), "n_iter": n_iter + 1})
        logger.log({"loss/dis": self.loss_dis_gp.item(), "n_iter": n_iter + 1})
        logger.log({"age_diff": self.age_diff.item(), "n_iter": n_iter + 1})
        logger.log({"dis/realism_A_modif": self.realism_a_modif.mean().item(), "n_iter": n_iter + 1})
        logger.log({"dis/realism_B": self.realism_b.mean().item(), "n_iter": n_iter + 1})
    ...
    def save_checkpoint(self, n_epoch, log_dir):
        ...
        torch.save(checkpoint_state, '{:s}/checkpoint/checkpoint'.format(log_dir))
        if (n_epoch+1) % 10 == 0 :
