...
parser.add_argument('--config', type=str, default='vnparams', help='Path to the config file.')
parser.add_argument('--dataset_path', type=str, default='../data/VNfaces/', help='dataset path')
parser.add_argument('--label_file_path', type=str, default='../data/vn_celeb_age_label.npy', help='label file path')
parser.add_argument('--vgg_model_path', type=str, default='../weights/dex_imdb_wiki.caffemodel.pt', help='pretrained age classifier')
...
parser.add_argument('--checkpoint', type=str, default='../weights/checkpoint', help='checkpoint file path')
...
with open('../configs/' + opts.config + '.yaml', 'r') as config_file:
...
for n_epoch in tqdm(range(epoch_0, epoch_0+epochs)):
    ...
    for i, list_A in tqdm(enumerate(loader_A)):
